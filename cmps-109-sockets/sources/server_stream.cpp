...
int main (int argc,char ** argv)
{
	...
	int newsock = accept(sock, (struct sockaddr *)&clientAddr,&sin_size); // Waiting for a client

	if ( newsock < 1 ) terminate_with_error("Error Accepting Socket",0);
	else
	{
		// Sets a zeroes out a buffer
		int maxBytes = 1024;
		char buffer[maxBytes];
		memset(buffer,0,maxBytes);

		int bytes_read = recv(newsock,buffer,maxBytes,0); // Reads the number of bytes from the client's buffer

		if ( bytes_read > 0 ) printf("Received Message from %s:%d\n%s\n",(char *)inet_ntoa(clientAddr.sin_addr),clientAddr.sin_port,buffer);
		else perror("Error!! Receiving Message from :");
		close(newsock);
	}
	close(sock);
}
